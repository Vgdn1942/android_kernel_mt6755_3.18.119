...
s32 do_i2c_read(struct i2c_msg * msgs, u16 addr, u8 * buffer, s32 len)
{
	s32 ret = -1;
	s32 pos = 0;
	s32 data_length = len;
...
s32 do_i2c_write(struct i2c_msg * msg, u16 addr, u8 * buffer, s32 len)
{
	s32 ret = -1;
	s32 pos = 0;
	s32 data_length = len;
...
#ifdef GTP_WRITE_CFG_FOR_KEWEI_U1
...
//#else /* DRIVER NOT SEND CONFIG */
#if !defined(GTP_WRITE_CFG) && !defined(CTP_FOR_HAND)
	cfg_len = GTP_CONFIG_MAX_LENGTH;
	ret = gt1x_i2c_read(GTP_REG_CONFIG_DATA, gt1x_config, cfg_len);
	if (ret < 0) {
		return ret;
	}
#endif /* END GTP_WRITE_CFG */
	...
	ret = gt1x_send_cfg(gt1x_config, gt1x_cfg_length);
	GTP_INFO("[geng_gt1x] send_cfg ret = %d",ret);
	return ret;
}

#if defined(CTP_FOR_HAND)
s32 gt1x_write_hand_cfg(void)
{
	static DEFINE_MUTEX(mutex_cfg);
	int i;
	...
	const u8 cfg_grp0[] = GTP_CFG_FOR_HAND_GROUP0;
	const u8 cfg_grp1[] = GTP_CFG_FOR_HAND_GROUP1;
	const u8 cfg_grp2[] = GTP_CFG_FOR_HAND_GROUP2;
	const u8 cfg_grp3[] = GTP_CFG_FOR_HAND_GROUP3;
	const u8 cfg_grp4[] = GTP_CFG_FOR_HAND_GROUP4;
	const u8 cfg_grp5[] = GTP_CFG_FOR_HAND_GROUP5;
	const u8 *cfgs[] = {
		cfg_grp0, cfg_grp1, cfg_grp2,
		cfg_grp3, cfg_grp4, cfg_grp5
	};
	...
	gt1x_config[cfg_len - 3] = (checksum >> 8) & 0xFF;
	gt1x_config[cfg_len - 2] = checksum & 0xFF;
	gt1x_config[cfg_len - 1] = 0x01;

	while (retry++ < 5) {
		ret = gt1x_i2c_write(GTP_REG_CONFIG_DATA, gt1x_config, cfg_len);
		if (!ret) {
			msleep(200);	/* at least 200ms, wait for storing config into flash. */
            mutex_unlock(&mutex_cfg);
			GTP_DEBUG("Send config successfully!");
...
/* start handle current event and pre-event */
	...
	if (CHK_BIT(cur_event, BIT_TOUCH_KEY))
	{
		...
		{
			key_x=60;
			key_y=TPD_RES_Y*850/800;
		}
		else if(key_value & 0x02)
		{
		#if defined TOUCH_KEY_FOR_KEWEI_U3
			key_x=180;
		#else
			key_x=300;
		#endif
