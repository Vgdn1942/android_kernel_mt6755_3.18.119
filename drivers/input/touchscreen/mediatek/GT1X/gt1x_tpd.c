...
static s32 i2c_write_mtk(u16 addr, u8 * buffer, s32 len)
{
	...
	ret = do_i2c_write(&msg, addr, buffer, len);
	return ret;
}
...
static s32 i2c_read_mtk(u16 addr, u8 * buffer, s32 len)
{
	...
	ret = do_i2c_read(msgs, addr, buffer, len);
	return ret;
}
...
node = of_find_matching_node(node, touch_of_match);
	...
	if(node){
		...
		{
GTP_INFO("[geng_gt1x] EINTF_TRIGGER_RISING");
...
GTP_INFO("[geng_gt1x] EINTF_TRIGGER_FALLING");
...
static int gt1x_get_fw_ver(void)
{
	...
	for(i=0;i<3;i++)
	{
		if(gt1x_i2c_read(GTP_REG_CONFIG_DATA, &ver, 1) < 0)
		{
...
{
		//int err = 0;
	printk("[chenpeng] enter GT1X: tpd_handler \n");
		if(0 == temp)
		{
			// ljj:驱动下发NORMAL模式固件。
			printk("[chenpeng]  TPD_SET_CFG_FOR_NORMAL  ~~~  \n");
			gt1x_init_panel();
			//err = gt1x_init_panel;
			//if (err < 0) {
			//	TPD_DEBUG("[chenpeng] fail to write tpd normal cfg %d\n", err );
			//}
		}else if(1 == temp)
		{
			// ljj:驱动下发手套模式固件。
			printk("[chenpeng]  TPD_SET_CFG_FOR_HAND  ~~~  \n");
			gt1x_write_hand_cfg();
			//err = gt1x_init_panel;
			//if (err < 0) {
			//	TPD_DEBUG("[chenpeng] fail to write tpd hand cfg %d\n", err );
			//}
		}else{
