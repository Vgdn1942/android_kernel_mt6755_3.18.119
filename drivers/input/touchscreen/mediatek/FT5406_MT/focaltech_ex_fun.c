...
extern unsigned int ft_irq;

u8 *ft_gpDMABuf_va = NULL;
...
{
	#ifdef AGOLD_TPD_FT_AUTO_UPGRADE
		#if defined(AGOLD_DFL_C221_FOR_TRULY)
		#include  "FT5436I_truly_app.i"
		#elif defined(AGOLD_LEMU_55C2K_FOR_GESTURE)
		#include  "FT5436I_lemu_truly_app.i"
		#endif
	#endif
};
...
*/
/*
int fts_i2c_Read_byte(struct i2c_client *client, char reg, int writelen, char *value)
{
	int ret = 0;
	u8 buf[2];
	buf[0]=reg;
	...
	{
		printk("[FTS] write reg[0x%x] fail, ret = %d\n",reg,ret);
	}
	
	if((ret = i2c_master_recv(client, value, 1)) < 0)
	{
		printk("[FTS] read reg[0x%x] fail, ret = %d\n",reg,ret);
	}
	return ret;		
}
*/
int fts_i2c_Write_byte(struct i2c_client *client, char reg, int writelen, char *value)
{
	int ret = 0;
	u8 buf[2];
	buf[0]=reg;
	...
	if((ret = i2c_master_send(client, buf, 2)) < 0)
	{
		printk("[AGold Bruce] write reg[0xc0=0x%x] value[0x%x] fail, ret = %d\n",reg,buf[1],ret);
	}
	printk("[FTS] write reg[0xc0=0x%x] value[0x%x] , ret = %d\n",reg,buf[1],ret);
	return ret;		
}
		    ...
		    int writelen, char *readbuf, int readlen)
{
	...
	if(writelen!=0)
	{
		...
		else
		{
			...
			{
				ft_gpDMABuf_va[i] = writebuf[i];
			}

			client->addr = (client->addr & I2C_MASK_FLAG) | I2C_DMA_FLAG;
		
			if((ret=i2c_master_send(client, (unsigned char *)gpDMABuf_pa, writelen))!=writelen)
				dev_err(&client->dev, "###%s i2c write len=%x,buffaddr=%llu\n", __func__,ret,gpDMABuf_pa);
	...
	if(readlen!=0)
	{
		...
		else
		{
			...
			client->addr = (client->addr & I2C_MASK_FLAG) | I2C_DMA_FLAG;
			ret = i2c_master_recv(client, (unsigned char *)gpDMABuf_pa, readlen);

			for(i = 0; i < readlen; i++)
	        {
	            readbuf[i] = ft_gpDMABuf_va[i];
	        }
		client->addr = client->addr & I2C_MASK_FLAG &(~ I2C_DMA_FLAG);
...
int fts_i2c_Write(struct i2c_client *client, char *writebuf, int writelen)
{
	int ret = 0;
	int i = 0;
	//printk("fts_i2c_Write  %d  %x  %x %x %x",writelen,writebuf,ft_gpDMABuf_va,gpDMABuf_pa,client);
...
*/
	if (2 == num_read_chars) 
	{
		...
		if (fts_read_reg(client, regaddr, &regvalue) < 0)
			printk( "Could not read the register(0x%x)\n",regaddr);
		else
			printk("the register(0x%x) is 0x%x\n",
					regaddr, regvalue);
	} 
	else 
	{
		...
		if (fts_write_reg(client, regaddr, regvalue) < 0)
			printk("Could not write the register(0x%x)\n",
							regaddr);
		else
			printk("Write 0x%x into register(0x%x) successful\n",
							regvalue, regaddr);
	}
...
static ssize_t fts_fwupgradeapp_store(struct device *dev,struct device_attribute *attr,const char *buf, size_t count)
{
	...
	{
		printk("FTP firmware upgrade success!\n");
	}
	else
	{
		printk("FTP firmware upgrade fail!\n");
	}
...
{
	int err;
	ft_gpDMABuf_va = (u8 *)dma_alloc_coherent(&tpd->dev->dev, FTS_DMA_BUF_SIZE, &gpDMABuf_pa, GFP_KERNEL);
	...
	printk("FTP: gpDMABuf_pa=%p,val=%llu val2=%p\n",&gpDMABuf_pa,gpDMABuf_pa,(unsigned char *)gpDMABuf_pa);
...
void fts_release_sysfs(struct i2c_client *client)
{
	...
	if(ft_gpDMABuf_va)
	{
		dma_free_coherent(&tpd->dev->dev, FTS_DMA_BUF_SIZE, ft_gpDMABuf_va, gpDMABuf_pa);
		ft_gpDMABuf_va = NULL;
		gpDMABuf_pa = 0;
	}
