...
{
    struct cdev cdev;
    //struct semaphore ft_rw_i2c_mutex;
...
//TPD_DEBUG("ft_rw_iic_drv_RDWR 6  %d %x  %x\n",ret,data_ptrs,i2c_rw_msg);
    if (ret < 0) {
        int j;
        for (j=0; j<i; ++j)
...
//TPD_DEBUG("ft_rw_iic_drv_RDWR 7\n");
    for (i=0; i< i2c_rw_queue.queuenum; i++) {
        //TPD_DEBUG("ft_rw_iic_drv_RDWR 8  %d  %x  %x  %d\n",i2c_rw_msg[i].flag,i2c_rw_msg[i].buf,data_ptrs[i],i2c_rw_msg[i].length);
...
//TPD_DEBUG("ft_rw_iic_drv_RDWR 10\n");
    return ret;
...
static long ft_rw_iic_drv_ioctl(struct file *filp, unsigned int cmd, unsigned long arg)
{
    int ret = 0;
    struct ft_rw_i2c_dev *ftdev = filp->private_data;
    ftdev = filp->private_data;
    //TPD_DEBUG("ft_rw_iic_drv_ioctl\n");
//  down(&ft_rw_i2c_dev_tt->ft_rw_i2c_mutex);
    //TPD_DEBUG("====ft_rw_iic_drv_ioctl: client = %x\n",ftdev->client);
    switch (cmd)
    {
    case FT_I2C_RW:
...
//  up(&ft_rw_i2c_dev_tt->ft_rw_i2c_mutex);
    //TPD_DEBUG("++++ft_rw_iic_drv_ioctl\n");
    return ret; 
}
...
static void ft_rw_iic_drv_setup_cdev(struct ft_rw_i2c_dev *dev, int index)
{
    ...
    TPD_DEBUG("%s: 2.client=%p  \n",__func__,dev->client);
    err = cdev_add(&dev->cdev, devno, 1);
    if (err)
        TPD_DEBUG(KERN_NOTICE "Error %d adding LED%d", err, index);
}
*/
static int ft_rw_iic_drv_myinitdev(struct i2c_client *client)
{
    ...
    TPD_DEBUG("ft_rw_iic_drv_myinitdev: client=%p \n",ft_rw_i2c_dev_tt->client);
    ...
    TPD_DEBUG("%s: client=%p  \n",__func__,ft_rw_i2c_dev_tt->client);
    if (err)
        TPD_DEBUG(KERN_NOTICE "Error %d adding LED", err);
    
    
    //ft_rw_iic_drv_setup_cdev(ft_rw_i2c_dev_tt, 0); 
    //TPD_DEBUG("===ft_rw_iic_drv_myinitdev: client=%x \n",ft_rw_i2c_dev_tt->client);
