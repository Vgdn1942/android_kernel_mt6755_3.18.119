...
static s32 gup_prepare_fl_fw(char *path, st_fw_head *fw_head)
{
    ...
    {
        GTP_ERROR("Inconsistent firmware size. File size: %u, default fw size: %zu", update_msg.fw_total_len, sizeof(gtp_hotknot_calibration_section0));
        set_fs(update_msg.old_fs);
        _CLOSE_FILE(update_msg.file);
        return FAIL;
    }
