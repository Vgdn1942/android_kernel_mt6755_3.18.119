...
//liukangping@wind-mobi add sleep begin 20160504
#include <linux/notifier.h>
#include <linux/fb.h>
//liukangping@wind-mobi add sleep end 20160504
#include <linux/completion.h>
#include <mt_spi.h>
#include "gfx1xm_spi.h"
...
struct device_node *finger_irq_node;
...
int gfx1xm_power_on(struct gfx1xm_dev *gfx1xm_dev, bool onoff)
{
	...
	if(onoff) 
	{
		if (0 == gfx1xm_dev->poweron)
		{
			...
			spi_dts_gpio_select_state(DTS_GPIO_STATE_FINGER_POWER1);
	...
	else 
	{
		...
		{		
			...
			spi_dts_gpio_select_state(DTS_GPIO_STATE_FINGER_POWER0);
...
/* devfs                                                                */
/* -------------------------------------------------------------------- */
static ssize_t gfx1xm_debug_show(struct device *dev, struct device_attribute *attr, char *buf)
{
	return (sprintf(buf, "%d\n", g_debug_level));
}
static ssize_t gfx1xm_debug_store(struct device *dev, struct device_attribute *attr, const char *buf, size_t count)
{
	int g_debug = 0;
 ...
 **************************************************************/
int gfx1xm_spi_read_bytes(struct gfx1xm_dev *gfx1xm_dev, u16 addr, u32 data_len, u8 *rx_buf)
{
	struct spi_message msg;
	struct spi_transfer *xfer;
...
static ssize_t gfx1xm_write(struct file *filp, const char __user *buf, size_t count, loff_t *f_pos)
{
    struct gfx1xm_dev *gfx1xm_dev = filp->private_data;
    ssize_t			status = 0;
...
static long gfx1xm_ioctl(struct file *filp, unsigned int cmd, unsigned long arg)
{
    ...
    switch(cmd) 
    {
		//[agold][xfl][20160425][for factory test]
		case GFX1XM_IOC_SPI_TEST:
				printk("func:%s, line:%d\n", __func__, __LINE__);
				retval = gfx1xm_check_9p_chip(&gfx1xm);
				break;
		case GFX1XM_IOC_CMD:
		...
		case GFX1XM_IOC_SLEEP:
			...
			mutex_unlock(&gfx1xm_dev->fb_lock);
			break;
		//liuakangping@wind-mobi.com add sleep begin 20160504
		case GFX1XM_IOC_PM_FBCABCK:
		printk("wby 001 go to ioctl fb_notify-----tsy\n");
		__put_user(gfx1xm_dev->fb_black, (u8 __user *) arg);
		break;
		//liuakangping@wind-mobi.com add sleep end 20160504
...
static int gfx1xm_event_handler(void *para)
{
	...
	do{
		...
		if (wakeup_spi) {
			printk("gfx1xm_event_handler: wakeup_spi = true \n");
			continue;
		}
...
int gfx1xm_config_update(struct gfx1xm_dev *gfx1xm_dev)
{
	...
	{		
		...
		{	//default config is config318_list[0].config[0].buffer
	...
	else if(chip_version_config==0x36)
	{
		...
		{	//default config is config316_list[0].config[0].buffer
...
static int gfx1xm_check_9p_chip(struct gfx1xm_dev *gfx1xm_dev)
{
	...
	do{
		...
		gfx1xm_debug(DEFAULT_DEBUG,"%s,9p chip version is 0x%02X, 0x%02X, 0x%02X, 0x%02X\n", __func__, reg_value[GFX1XM_RDATA_OFFSET], 
		           reg_value[GFX1XM_RDATA_OFFSET+1], reg_value[GFX1XM_RDATA_OFFSET+2], reg_value[GFX1XM_RDATA_OFFSET+3]);
                                        ...
                                        unsigned long val, void *data)
{
        ...
        if (val != FB_EARLY_EVENT_BLANK)
                return 0;
        pr_info("wby [info] %s go to the goodix_fb_state_chg_callback value = %d\n",
                __func__, (int)val);
        gfx1xm_dev = container_of(nb, struct gfx1xm_dev, notifier);
        if (evdata && evdata->data && val == FB_EARLY_EVENT_BLANK && gfx1xm_dev) {
                blank = *(int *)(evdata->data);
                switch (blank) {
                case FB_BLANK_POWERDOWN:
                    pr_info("wby %s powerdown\n", __func__);
                        gfx1xm_dev->fb_black = 0;
#if GFX1XM_FASYNC
                        if (gfx1xm_dev->async) {
							//pr_info("wby %s powerdown 000 clean buf\n", __func__);
							gfx1xm_spi_write_byte(gfx1xm_dev, GFX1XM_BUFFER_STATUS, 0x00); //clean buf
                        	kill_fasync(&gfx1xm_dev->async, SIGIO, POLL_IN);
							pr_info("wby %s kill sigio power down\n", __func__);
                        }
#endif
                       
                        break;
                case FB_BLANK_UNBLANK:
                    pr_info("wby %s unblank\n", __func__);   
                            gfx1xm_dev->fb_black = 1;
#if GFX1XM_FASYNC
                         if (gfx1xm_dev->async) {
							//pr_info("wby %s unblank 111 clean buf\n", __func__);
			    			gfx1xm_spi_write_byte(gfx1xm_dev, GFX1XM_BUFFER_STATUS, 0x00); //clean buf
                            kill_fasync(&gfx1xm_dev->async, SIGIO, POLL_IN);
							pr_info("wby %s kill sigio unblank\n", __func__);
                         }
#endif                       
                        break;
                default:
...
#error "have't define any product for fw"
	...
	if (minor < N_SPI_MINORS) 
	{
		...
		dev = device_create(gfx1xm_spi_class, &spi->dev, gfx1xm_dev->devt, gfx1xm_dev, DEV_NAME);
	...
	mutex_unlock(&device_list_lock);
	//liuakangping@wind-mobi.com add sleep begin 20160504
	gfx1xm_dev->notifier = goodix_noti_block;
	fb_register_client(&gfx1xm_dev->notifier);
	//liuakangping@wind-mobi.com add sleep end 20160504
	...
	if(finger_irq_node)
	{
		...
		if(request_irq(gfx1xm_dev->spi->irq,gfx1xm_irq,IRQF_TRIGGER_RISING,"hall_2-eint",NULL))
		{
...
static int gfx1xm_suspend_test(struct device *dev)
{
    ...
    wakeup_spi = true;
    disable_irq(gfx1xm_dev->spi->irq);
    printk("gfx1xm: %s irq is disabled!\n",__func__);
    printk("gfx1xm: %s\n", __func__);
    return 0;
}
