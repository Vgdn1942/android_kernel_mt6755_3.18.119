...
#ifdef AGOLD_FINGERPRINT_COMPATIBLE
#include "../fp.h"
#include <mt_spi.h>
extern int fp_load_status;
extern int mt6755_dts_register_platform(void);
#endif
...
DEVICE_ATTR(version, S_IRUGO | S_IWUSR, sunwave_version_show, NULL);
DEVICE_ATTR(debug, S_IRUGO | S_IWUSR, sunwave_debug_show, sunwave_debug_store);
DEVICE_ATTR(chip_info, S_IRUGO | S_IWUSR, sunwave_chip_info_show, NULL);
...
static struct attribute_group sunwave_attr_group = {
...
sunwave_dev_read(struct file* filp, char __user* buf, size_t count, loff_t* f_pos)
{
    ...
    if (sunwave_dev->finger && sunwave_dev->finger->write_then_read == 0) {
        ...
        if (status >= 0) {
            ...
            sw_dbg("ret %x missing = %ld status = %ld", (buf[status - 2] << 8) | buf[status - 1], missing, status);
...
static int __devinit sunwave_dev_probe(struct spi_device* spi)
{
    ...
    //add by Sea 20160505
#if defined(AGOLD_FINGERPRINT_COMPATIBLE) || defined(__SUNWAVE_DETECT_ID_EN)
		
    #if defined(AGOLD_FINGERPRINT_COMPATIBLE)
    if (sunwave_dev->finger && sunwave_dev->finger->com_mode) {
        status = sunwave_dev->finger->com_mode(&sunwave_dev->spi,DMA_TRANSFER);
        if (status < 0) {
            sw_err("[MaJian]set com_mode fail %d\n", status) ;
            return status;
        }
        ...
        {
            sw_dbg("[MaJian]%s %d: set com_mode success!\n",__func__,__LINE__);
        }
    }
    #endif   
...
{	
	...
	if (status < 0)
	{
		sw_err("%s : cannot spi register : %d\n", __func__, status);
	...
	{
		spi_unregister_driver(&sunwave_dev_spi_driver);
		sw_err("%s : unregister spi driver: %d\n", __func__, __LINE__);
	}
	
    return 0; 
}

static struct fp_driver_t fp_device_driver = 
{
	.fp_device_name = "sunwave",
	.fp_local_init = fp_local_init,
};
#endif
...
#ifdef AGOLD_FINGERPRINT_COMPATIBLE
	sw_info("%s, %d\n", __func__, __LINE__);
	mt6755_dts_register_platform();
	sw_info("%s, %d\n", __func__, __LINE__);
	
	status = fp_driver_add(&fp_device_driver);
	if(status != 0)
   	{
  		printk("FP can't add spi driver.\n");
      	return -1;
	}
	else
	{
		printk("FP add spi driver success.\n");
	}
#else
