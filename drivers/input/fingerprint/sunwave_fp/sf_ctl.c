...
//#include "../../../misc/mediatek/tkcore/core/tee_fp.h"
...
static struct mt_spi_t *mt_spi = NULL;
...
static const char *sf_pinctrl_state_names[SF_PIN_STATE_MAX] = {
    "finger_power_high", "finger_power_low", "finger_rst_low", "finger_rst_high", "finger_eint",
};
...
static int sf_spi_clock_enable(struct spi_device *spi, bool on)
{
    int err = 0;
...
static long sf_ctl_ioctl(struct file *filp, unsigned int cmd, unsigned long arg)
{
    ...
    switch (cmd) {
        ...
        case SF_IOC_ENABLE_SPI_CLK: {
            // TODO:
            sf_spi_clock_enable(sf_spi,true);
            break;
        }

        case SF_IOC_DISABLE_SPI_CLK: {
            // TODO:
            sf_spi_clock_enable(sf_spi,false);
            break;
        }
...
static int sf_probe(struct spi_device *spi)
{
    ...
    if (sf_read_sensor_id() < 0) {
        ...
        sf_spi_clock_enable(sf_spi,false);//chenlw add
        return -1;
    }
