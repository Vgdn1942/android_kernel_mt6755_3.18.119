...
#define Agold_add_for_compatible

/* MTK mt6975_evb reset & interrupt pin */
#if !defined(Agold_add_for_compatible)

#define GPIO_FINGER_RESET_PIN	 	 55
#define GPIO_FINGER_PWR_EN_PIN  	 100
...
int fp_io_get_one_image(struct fp_data *fp, u8 *buf, u8 *image_buf)
{
	...
	if (copy_to_user((u8 __user *) (uintptr_t) image_buf, work_buf+3, read_count)) 
	{
...
int Interrupt_Init(int int_mode, int detect_period, int detect_threshold)
{
	...
	printk("fps_ints.drdy_irq_flag:%d\n", fps_ints.drdy_irq_flag);
	if(fps_ints.drdy_irq_flag == DRDY_IRQ_DISABLE)
	{        
		printk("fps_ints.drdy_irq_flag == DRDY_IRQ_DISABLE\n");
		...
		if (node)
		{
			...
			else if(int_mode == LEVEL_TRIGGER_LOW) 
			{
				...
				printk("int_mode == LEVEL_TRIGGER_LOW \n");
			}
			if (ret > 0)
			{
...
#if !defined(Agold_add_for_compatible)	
	pr_info("%s\n", __func__);
	gpio_set_value(GPIO_FINGER_RESET_PIN, 0);
	msleep(30);
	gpio_set_value(GPIO_FINGER_RESET_PIN, 1);
	msleep(20);
#else
	pr_info("%s\n", __func__);
...
static long fp_ioctl(struct file *filp, unsigned int cmd, unsigned long arg)
{
	...
	if (ioc->opcode == FP_GET_ONE_IMG) 
	{
		...
		if (retval < 0) 
		{
		 ...
		 spi->controller_data = (void *) &spi_conf;          // mtk configuration fae corey.liu add 20170330
	     spi_setup(spi);
	}
	...
	if (ioc->opcode == INT_TRIGGER_INIT) 
	{
		...
		retval = Interrupt_Init((int)ioc->pad[0], (int)ioc->pad[1], (int)ioc->pad[2]);
...
/*-------------------------------------------------------------------------*/
#if defined(Agold_add_for_compatible)
static int spi_dts_gpio_init(struct fp_data *fp)
{

	int ret = 0;

	struct device_node *node;
	printk("[MaJian][%s][line:%d] test 0 \n", __func__, __LINE__);
	
	node = of_find_compatible_node(NULL, NULL,"mediatek,fingerprint");
	if(node)
	{
		finger_rst_low = pinctrl_lookup_state(fp->pin_ctrl, "finger_rst_low");
		...
		finger_rst_high = pinctrl_lookup_state(fp->pin_ctrl, "finger_rst_high");
		...
		finger_power_low = pinctrl_lookup_state(fp->pin_ctrl, "finger_power_low");
		...
		finger_power_high = pinctrl_lookup_state(fp->pin_ctrl, "finger_power_high");
...
{
	struct fp_data *fp = &fp_et320;
	int status;
	unsigned long minor;
	int ret;		
	int res;
	uint8_t read_val0,read_val1;
...
//[agold][MaJian][start]20160307 
#if defined(Agold_add_for_compatible)
	spi->dev.of_node=of_find_compatible_node(NULL, NULL, "mediatek,fingerprint");
	fp->pin_ctrl = devm_pinctrl_get(&spi->dev);
    if (IS_ERR(fp->pin_ctrl)) 
    {
    	ret = PTR_ERR(fp->pin_ctrl);
    	dev_err(&spi->dev, "fwq Cannot find fp pctrl!\n");
    	return ret;
    }

	spi_dts_gpio_init(fp);
	pinctrl_select_state(fp_pinctrl,finger_rst_low);	
	msleep(20);	
	pinctrl_select_state(fp_pinctrl,finger_rst_high);
	msleep(20);	
	...
	ret = gpio_request(GPIO_FINGER_RESET_PIN, "ars_rst");
	if (ret)
	{
		printk("[ars3022]  : gpio_request (%d)fail\n", GPIO_FINGER_RESET_PIN);
	}
	
	ret = gpio_direction_output(GPIO_FINGER_RESET_PIN, 0);
	if (ret)
	{
	...
	ret = gpio_request(GPIO_FINGER_PWR_EN_PIN, "ars_pwr");
	if (ret)
	{
		printk("[ars3022]  : gpio_request (%d)fail\n", GPIO_FINGER_PWR_EN_PIN);
	}
	
	ret = gpio_direction_output(GPIO_FINGER_PWR_EN_PIN, 0);
	if (ret)
	{
		printk("[ars3022]gpio_direction_output (%d)fail\n",GPIO_FINGER_PWR_EN_PIN);
	}
	gpio_set_value(GPIO_FINGER_PWR_EN_PIN, 1);

#endif
//[agold][MaJian][end]20160307 
	...
	res = fp_read_register(fp, 0x11, &read_val0);
	if(!res)
	{
		DEBUG_PRINT("%s : read vendor id 0x11 read_val0= 0x%2x\n", __func__,read_val0);
	}
	res = fp_read_register(fp, 0x13, &read_val1);
	if(!res)
	{
		DEBUG_PRINT("%s : read vendor id 0x13 read_val1= 0x%2x\n", __func__,read_val1);
	}
	
	
	if ((read_val0 == 0x3F) && ( read_val1 == 0x8F))
	{
		DEBUG_PRINT("%s : read vendor id success!\n", __func__);
	}
	else
	{
		DEBUG_PRINT("%s : read vendor id fail!\n", __func__);
		return -EINVAL;		
	}
...
{
	DEBUG_PRINT("%s\n", __func__);
	//gpio_free(GPIO_FINGER_RESET_PIN);
