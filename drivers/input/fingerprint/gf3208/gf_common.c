...
static int gf_get_gpio_dts_info(struct gf_device *gf_dev)
{
	...
	gf_dev->pins_irq = pinctrl_lookup_state(gf_dev->pinctrl_gpios, "fp_state_eint_as_int");
	...
	gf_dev->pins_default = pinctrl_lookup_state(gf_dev->pinctrl_gpios, "fp_default");
	if (IS_ERR(gf_dev->pins_default)) {
		ret = PTR_ERR(gf_dev->pins_default);
		pr_info("fwq Cannot find nasri_fingerprint pinctrl default %d!\n", ret);
	}

	gf_dev->pwr_output0 = pinctrl_lookup_state(gf_dev->pinctrl_gpios, "finger_power_low");
	if (IS_ERR(gf_dev->pwr_output0)) {
		ret = PTR_ERR(gf_dev->pwr_output0);
		pr_info("fwq Cannot find pinctrl finger_power_low!\n");
		return ret;
	}
	
	gf_dev->pwr_output1 = pinctrl_lookup_state(gf_dev->pinctrl_gpios, "finger_power_high");
	if (IS_ERR(gf_dev->pwr_output1)) {
		ret = PTR_ERR(gf_dev->pwr_output1);
		pr_info("fwq Cannot find  pinctrl finger_power_high!\n");
		return ret;
	}
...
static void gf_hw_power_enable(struct gf_device *gf_dev, u8 onoff)
{
	...
	static int enable = 1;
	if (onoff && enable) {	
		pinctrl_select_state(gf_dev->pinctrl_gpios, gf_dev->pwr_output1);		
		enable = 0;
	}
	else if (!onoff && !enable) {	
		pinctrl_select_state(gf_dev->pinctrl_gpios, gf_dev->pwr_output0);
		enable = 1;
	}
}
...
//[agold][xfl][20170630]
static unsigned int gf_get_chip_id(struct gf_device *gf_dev)
{
	...
	chip_id = ((chip_id_2<<16)|(chip_id_1)) >> 8;
	if(0x00002202 != chip_id)	// atb 20161222 goodix 0x00002202
	{
		gf_debug(ERR_LOG, "Finger Print: Have not used Goodix, return\n");
	}
	gf_debug(INFO_LOG, "%s: chip_id_hi = 0x%x,chip_id_low = 0x%x, chip_id is 0x%08x \n", __func__, chip_id_2, chip_id_1, chip_id);
...
static long gf_ioctl(struct file *filp, unsigned int cmd, unsigned long arg)
{
	...
	*/
	...
	//[agold][xfl][20170630]
	case GF_IOC_SPI_GET_CHIP_ID:
		retval = gf_get_chip_id(gf_dev);
	break;
