...
int gf_spi_read_bytes_gf358(struct gf_device* gf_dev, unsigned short addr, unsigned short data_len,
...
int gf_spi_read_word(struct gf_device* gf_dev, unsigned short addr, unsigned short* value)
{
    int status = 0;
    ...
    status = gf_spi_read_bytes_gf358(gf_dev, addr, 2, p);
    q = p + GF_RDATA_OFFSET;
    *value = ((unsigned short)(q[0]<<8)) | q[1];
