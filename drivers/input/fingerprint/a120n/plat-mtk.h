...
extern int ma_finger_set_power(int cmd);
extern int ma_finger_set_reset(int cmd);
extern int ma_finger_set_spi_mode(int cmd);
...
{		
	...
	ma_finger_set_power(1);
	ma_finger_set_reset(1);
	ma_finger_set_spi_mode(1);
...
int plat_request_irq(unsigned int irq, irq_handler_t handler, unsigned long flags, const char *name, void *dev) 
{	
	u32 ints[2] = {0};
	struct device_node *node = NULL;
	const char*tname = "mediatek,fingerprint";
