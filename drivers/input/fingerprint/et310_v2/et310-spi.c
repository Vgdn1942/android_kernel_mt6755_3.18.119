...
//#define FP_SPI_pr_info
#ifdef FP_SPI_pr_info
#define pr_info_PRINT(fmt, args...)	pr_err(fmt, ## args)
#else
#define pr_info_PRINT(fmt, args...)
#endif
...
extern void fingerprint_eint_work_func(struct work_struct *work);
...
unsigned int et310_irq;
static int et310_power_pin_num = -1;
static int et310_rst_pin_num = -1;
int et310_irq_registration(void);

struct device_node *et310_irq_node;
...
int fp_io_read_register(struct fp_data *fp, u8 *addr, u8 *buf)
{
	...
	pr_info_PRINT("%s Read add_val = %x buf = 0x%x\n", __func__, addrval, val);
...
int fp_io_write_register(struct fp_data *fp, u8 *buf)
{
	...
	pr_info_PRINT("%s write_len = %d\n", __func__, write_len);
	pr_info_PRINT("%s address = %x data = %x\n", __func__, val[0], val[1]);
...
int fp_read_register(struct fp_data *fp, u8 addr, u8 *buf)
{
	...
	{
		*buf = result[1];
		pr_info_PRINT("fp_read_register address = %x result = %x %x\n", addr, result[0], result[1]);
	} 
	else
	{
		pr_err("%s read data error status = %d\n", __func__, status);
...
int fp_eeprom_rdsr(struct fp_data *fp, u8 *buf)
{
	...
	pr_info_PRINT("%s address = %x buf = %x\n", __func__, FP_EEPROM_RDSR_OP, val);
...
int fp_eeprom_wrsr(struct fp_data *fp, u8 *buf)
{
	...
	if (copy_from_user(&val, (const u8 __user *) (uintptr_t) buf, 1)) 
	{
		pr_err("%s buffer copy_from_user fail\n", __func__);
		status = -EFAULT;
		return status;
...
int fp_eeprom_read(struct fp_data *fp, u8 *addr, u8 *buf, int read_len)
{
	...
	if (copy_from_user(&addrval, (const u8 __user *) (uintptr_t) addr, 1)) 
	{
		pr_err("%s buffer copy_from_user fail\n", __func__);
		status = -EFAULT;
		goto exit;
	...
	if (copy_to_user((u8 __user *) (uintptr_t) buf, read_value + 2, read_len)) 
	{
		pr_err("%s buffer copy_to_user fail status\n", __func__);
		status = -EFAULT;
		goto exit;
...
int fp_eeprom_write(struct fp_data *fp, u8 *buf, int write_len)
{
	...
	if (copy_from_user(write_value + 1, (const u8 __user *) (uintptr_t) buf, write_len + 1)) 
	{
		pr_err("%s buffer copy_from_user fail\n", __func__);
		status = -EFAULT;
		goto exit;
...
{
	gpio_set_value(et310_rst_pin_num, 0);
    msleep(10);
	gpio_set_value(et310_rst_pin_num, 1);
}
...
static long fp_ioctl(struct file *filp, unsigned int cmd, unsigned long arg)
{
	...
	if (ioc->opcode == FP_REGISTER_READ) 
	{
		...
		pr_info_PRINT("fp FP_REGISTER_READ\n");
	...
	{
		u8 *buf = ioc->tx_buf;
		pr_info_PRINT("fp FP_REGISTER_WRITE");
	...
	if (ioc->opcode == FP_GET_ONE_IMG) 
	{
		...
		pr_info_PRINT("fp FP_GET_ONE_IMG\n");
...
#ifdef CONFIG_COMPAT
static long fp_compat_ioctl(struct file *filp, unsigned int cmd, unsigned long arg)
{
...
static int fp_open(struct inode *inode, struct file *filp)
{
	...
	{
		pr_info("fp: nothing for minor %d\n", iminor(inode));
	}
...
int et310_irq_registration(void)
{
	...
	pr_info("Device Tree et310_irq_registration!");

	node = of_find_matching_node(node, et310_of_match);
	if (node) {
		...
		et310_irq = irq_of_parse_and_map(node, 0);

		ret = request_irq(et310_irq, (irq_handler_t) fingerprint_interrupt, IRQF_TRIGGER_RISING,"FINGERPRINT-eint", NULL);
		if (ret > 0) {
			ret = -1;
			pr_info("et310 request_irq IRQ LINE NOT AVAILABLE!.");
		}
	
	} else {
		pr_info("et310 request_irq can not find et310 eint device node!.");
		ret = -1;
	}
	enable_irq(et310_irq);
	pr_info("[%s]irq:%d, debounce:%d-%d, et310_irq:%d", __func__, et310_irq, ints[0], ints[1],et310_irq);
	return ret;
}

int et310_get_gpio_info(struct spi_device *spi)
{
	...
	if (node) 
	{
		ret = of_property_read_u32(node , "power_en_pin", &et310_power_pin_num);
		if (ret)
		{
			printk("[fpc]%s read node power_en_pin error!\n", __func__);
		}

		ret = of_property_read_u32(node , "rst_pin", &et310_rst_pin_num);
		if (ret)
		{
			printk("[fpc]%s read node rst_pin error!\n", __func__);
		}
		
		printk("[fpc]%s et310_power_pin_num = %d, et310_rst_pin_num0=%d!\n", __func__,et310_power_pin_num,et310_rst_pin_num);
...
static int fp_probe(struct spi_device *spi)
{
	...
	if (minor < N_SPI_MINORS) 
	{
		...
		dev = device_create(fp_class, &spi->dev, fp->devt, fp, "esfp0");
	...
	et310_get_gpio_info(spi);

	gpio_set_value(et310_power_pin_num, 1);
    msleep(100);
