...
#endif
#if defined(AGOLD_ACC_SET_DIRECTION)
	char acc_dir_tag[16];
	int  acc_direction;
#endif
#if defined(AGOLD_FACTORY_TEST_CONFIG)
	char factory_item_test_list[32];
#endif
};
...
#ifdef AGOLD_FACTORY_TEST_CONFIG
char* agold_get_factory_test_item_list(void)
{
	//strcpy(item_list,g_agold_hw_info.factory_item_test_list);
	return g_agold_hw_info.factory_item_test_list;
}
#endif

#if defined(AGOLD_ACC_SET_DIRECTION)
int agold_acc_get_direction(char *acc_dirTag)
{
	strcpy(acc_dirTag,g_agold_hw_info.acc_dir_tag);
	printk("[kernel] get_direction , acc_dirTag = %s,direction = %d\n", acc_dirTag,g_agold_hw_info.acc_direction);
	return g_agold_hw_info.acc_direction;
}
#endif
