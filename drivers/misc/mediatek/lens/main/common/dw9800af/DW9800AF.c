...
static int i2c_read(u8 a_u2Addr , u8 * a_puBuff)
{
	...
	LOG_INF("[DW9800AF]i2c_master_send i4RetValue - %x\n\n",i4RetValue);
	if (i4RetValue != 1) {
	    LOG_INF("[DW9800AF] I2C write failed!! \n");
	    return -1;
	}
	i4RetValue = i2c_master_recv(g_pstAF_I2Cclient, (char *)a_puBuff, 1);
	LOG_INF("[DW9800AF]i2c_master_recv i4RetValue - %x\n\n",i4RetValue);
	if (i4RetValue != 1) {
	    LOG_INF("[DW9800AF] I2C read failed!! \n");
	    return -1;
	}
    return 0;
}
...
static int s4AF_WriteReg(u16 a_u2Data)
{
	...
	char puSendCmd[3] = {0x03,(char)(a_u2Data >> 8), (char)(a_u2Data & 0xff)}; //DW9800 AF mode
