...
static int PA12201001_i2c_suspend(struct device *dev);
static int PA12201001_i2c_resume(struct device *dev);
...
static const struct dev_pm_ops PA12201001_pm_ops = {
	SET_SYSTEM_SLEEP_PM_OPS(PA12201001_i2c_suspend, PA12201001_i2c_resume)
};
#endif
...
/*----------------------------------------------------------------------------*/
static struct i2c_client *PA12201001_i2c_client;
static struct PA12201001_priv *PA12201001_obj;


static int PA12201001_local_init(void);
static int PA12201001_remove(void);
static int PA12201001_init_flag = -1;
...
static DEFINE_MUTEX(PA12201001_mutex);
...
int PA12201001_i2c_master_operate(struct i2c_client *client, char *buf, int count, int i2c_flag)
{
	int res = 0;
#ifndef CONFIG_MTK_I2C_EXTENSION
	struct i2c_msg msg[2];
#endif
	mutex_lock(&PA12201001_mutex);
	switch (i2c_flag) {
	case I2C_FLAG_WRITE:
	...
	case I2C_FLAG_READ:
		...
		break;
	default:
		APS_LOG("PA12201001_i2c_master_operate i2c_flag command not support!\n");
		break;
	}
	if (res < 0)
		goto EXIT_ERR;
	mutex_unlock(&PA12201001_mutex);
	return res;
EXIT_ERR:
	mutex_unlock(&PA12201001_mutex);
	APS_ERR("PA12201001_i2c_master_operate fail\n");
	return res;
}
...
int PA12201001_enable_ps(struct i2c_client *client, int enable)
{
	...
	if (enable == 1) {
		...
		APS_LOG("PA12201001_enable_ps enable_ps\n");
		databuf[0] = PA12201001_REG_PS_CONF1_2;
		res = PA12201001_i2c_master_operate(client, databuf, 2, I2C_FLAG_READ);
		if (res < 0) {
			APS_ERR("i2c_master_send function err\n");
			goto ENABLE_PS_EXIT_ERR;
		}
		APS_LOG("PA12201001_REG_PS_CONF1_2 value value_low = %x, value_high = %x\n", databuf[0], databuf[1]);
		databuf[2] = databuf[1];
		databuf[1] = databuf[0] & 0xFE;

		databuf[0] = PA12201001_REG_PS_CONF1_2;
		res = PA12201001_i2c_master_operate(client, databuf, 0x3, I2C_FLAG_WRITE);
		if (res < 0) {
			APS_ERR("i2c_master_send function err\n");
			goto ENABLE_PS_EXIT_ERR;
		}
		...
		APS_LOG("PA12201001_enable_ps disable_ps\n");
		databuf[0] = PA12201001_REG_PS_CONF1_2;
		res = PA12201001_i2c_master_operate(client, databuf, 2, I2C_FLAG_READ);
		if (res < 0) {
			APS_ERR("i2c_master_send function err\n");
...
/********************************************************************/
int PA12201001_enable_als(struct i2c_client *client, int enable)
{
	struct PA12201001_priv *obj = i2c_get_clientdata(client);
	int res = 0;
	u8 databuf[3];
	...
	if (enable == 1) {
		APS_LOG("PA12201001_enable_als enable_als\n");
		databuf[0] = PA12201001_REG_ALS_UV_CONF;
		res = PA12201001_i2c_master_operate(client, databuf, 2, I2C_FLAG_READ);
		if (res < 0) {
			APS_ERR("i2c_master_send function err\n");
		...
		APS_LOG("PA12201001_REG_ALS_UV_CONF value value_low = %x, value_high = %x\n", databuf[0], databuf[1]);
		...
		APS_LOG("PA12201001_enable_als disable_als\n");
		databuf[0] = PA12201001_REG_ALS_UV_CONF;
		res = PA12201001_i2c_master_operate(client, databuf, 2, I2C_FLAG_READ);
		if (res < 0) {
			APS_ERR("i2c_master_send function err\n");
...
/********************************************************************/
long PA12201001_read_ps(struct i2c_client *client, u16 *data)
{
	long res = 0;
	u8 databuf[2];
	struct PA12201001_priv *obj = i2c_get_clientdata(client);

	databuf[0] = PA12201001_REG_PS_DATA;
	res = PA12201001_i2c_master_operate(client, databuf, 2, I2C_FLAG_READ);
	if (res < 0) {
		APS_ERR("i2c_master_send function err\n");
	...
	if (atomic_read(&obj->trace) & CMC_TRC_DEBUG)
		APS_LOG("PA12201001_REG_PS_DATA value value_low = %x, value_high = %x\n", databuf[0], databuf[1]);
...
/********************************************************************/
long PA12201001_read_als(struct i2c_client *client, u16 *data)
{
	long res = 0;
	u8 databuf[2];
	struct PA12201001_priv *obj = i2c_get_clientdata(client);

	databuf[0] = PA12201001_REG_ALS_DATA;
	res = PA12201001_i2c_master_operate(client, databuf, 2, I2C_FLAG_READ);
	if (res < 0) {
		APS_ERR("i2c_master_send function err\n");
	...
	if (atomic_read(&obj->trace) & CMC_TRC_DEBUG)
		APS_LOG("PA12201001_REG_ALS_DATA value: %d\n", ((databuf[1] << 8) | databuf[0]));
...
static ssize_t PA12201001_show_config(struct device_driver *ddri, char *buf)
{
	...
	res = snprintf(buf, PAGE_SIZE, "(%d %d %d %d %d\n)threadhold_low=%d threadhold_high=%d\n",
		       ...
		       atomic_read(&PA12201001_obj->ps_mask), atomic_read(&PA12201001_obj->ps_thd_val),
		       atomic_read(&PA12201001_obj->ps_debounce), atomic_read(&PA12201001_obj->ps_thd_val_low),
		       atomic_read(&PA12201001_obj->ps_thd_val_high));
	return res;
}
...
static ssize_t PA12201001_show_ps(struct device_driver *ddri, char *buf)
{
	...
	res = PA12201001_read_ps(PA12201001_obj->client, &PA12201001_obj->ps);
	if (res)
		return snprintf(buf, PAGE_SIZE, "ERROR: %d\n", (unsigned int)res);
	else
		return snprintf(buf, PAGE_SIZE, "0x%04X\n", PA12201001_obj->ps);
...
static ssize_t PA12201001_show_reg(struct device_driver *ddri, char *buf)
{
	...
	for (_bIndex = 0; _bIndex < 0x0E; _bIndex++) {
		databuf[0] = _bIndex;
		res = PA12201001_i2c_master_operate(PA12201001_obj->client, databuf, 2, I2C_FLAG_READ);
		if (res < 0)
			APS_ERR("PA12201001_i2c_master_operate err res = %d\n", res);
...
static ssize_t PA12201001_show_status(struct device_driver *ddri, char *buf)
{
	...
	len +=
	    snprintf(buf + len, PAGE_SIZE - len, "MISC: %d %d\n", atomic_read(&PA12201001_obj->als_suspend),
		     atomic_read(&PA12201001_obj->ps_suspend));
...
static ssize_t PA12201001_store_alslv(struct device_driver *ddri, const char *buf, size_t count)
{
	if (!PA12201001_obj) {
		...
		memcpy(PA12201001_obj->als_level, PA12201001_obj->hw.als_level, sizeof(PA12201001_obj->als_level));
								   ...
								   PA12201001_obj->hw.als_level,
								   PA12201001_obj->als_level_num)) {
		APS_ERR("invalid format: '%s'\n", buf);
	}
	return count;
}
...
static ssize_t PA12201001_store_alsval(struct device_driver *ddri, const char *buf, size_t count)
{
	if (!PA12201001_obj) {
		...
		memcpy(PA12201001_obj->als_value, PA12201001_obj->hw.als_value, sizeof(PA12201001_obj->als_value));
								   ...
								   PA12201001_obj->hw.als_value,
								   PA12201001_obj->als_value_num)) {
		APS_ERR("invalid format: '%s'\n", buf);
	}
	return count;
}
...
static DRIVER_ATTR(send, S_IWUSR | S_IRUGO, PA12201001_show_send, PA12201001_store_send);
static DRIVER_ATTR(recv, S_IWUSR | S_IRUGO, PA12201001_show_recv, PA12201001_store_recv);
...
{
	int idx = 0, err = 0;
	int num = (int)(ARRAY_SIZE(PA12201001_attr_list));

	if (driver == NULL)
		return -EINVAL;

	for (idx = 0; idx < num; idx++) {
		err = driver_create_file(driver, PA12201001_attr_list[idx]);
		if (err) {
...
static int PA12201001_check_intr(struct i2c_client *client)
{
	...
	pr_warn("PA12201001_REG_PS_DATA value value_low = %x, value_reserve = %x\n", databuf[0], databuf[1]);

	databuf[0] = PA12201001_REG_INT_FLAG;
	res = PA12201001_i2c_master_operate(client, databuf, 2, I2C_FLAG_READ);
	if (res < 0) {
		APS_ERR("i2c_master_send function err res = %d\n", res);
		goto EXIT_ERR;
	...
	pr_warn("PA12201001_REG_INT_FLAG value value_low = %x, value_high = %x\n", databuf[0], databuf[1]);
	...
	if (databuf[1] & 0x02) {
		...
		res = -1;
		APS_ERR("PA12201001_check_intr fail databuf[1]&0x01: %d\n", res);
		goto EXIT_ERR;
	}
...
static void PA12201001_eint_work(struct work_struct *work)
{
	...
	pr_warn("PA12201001 int top half time = %lld\n", int_top_time);

	res = PA12201001_check_intr(obj->client);
	if (res != 0) {
		goto EXIT_INTR_ERR;
	} else {
		APS_LOG("PA12201001 interrupt value = %d\n", intr_flag);
...
/* eint request */
	if (PA12201001_obj->irq_node) {
		of_property_read_u32_array(PA12201001_obj->irq_node, "debounce", ints, ARRAY_SIZE(ints));
		...
		enable_irq_wake(PA12201001_obj->irq);
...
static int set_psensor_threshold(struct i2c_client *client)
{
	...
	databuf[0] = PA12201001_REG_PS_THDL;
	...
	databuf[0] = PA12201001_REG_PS_THDH;
...
static long PA12201001_unlocked_ioctl(struct file *file, unsigned int cmd, unsigned long arg)
{
	...
	case ALSPS_SET_PS_MODE:
		...
		if (enable) {
			err = PA12201001_enable_ps(obj->client, 1);
			if (err) {
			...
			err = PA12201001_enable_ps(obj->client, 0);
			if (err) {
	...
	case ALSPS_SET_ALS_MODE:
		...
		if (enable) {
			err = PA12201001_enable_als(obj->client, 1);
			if (err) {
			...
			err = PA12201001_enable_als(obj->client, 0);
			if (err) {
...
static long compat_PA12201001_unlocked_ioctl(struct file *filp, unsigned int cmd,
				unsigned long arg)
{
...
static const struct file_operations PA12201001_fops = {
	.owner = THIS_MODULE,
	.open = PA12201001_open,
	.release = PA12201001_release,
	.unlocked_ioctl = PA12201001_unlocked_ioctl,
#if IS_ENABLED(CONFIG_COMPAT)
	.compat_ioctl = compat_PA12201001_unlocked_ioctl,
#endif
};
...
static int PA12201001_init_client(struct i2c_client *client)
{
	...
	APS_LOG("PA12201001 ps PA12201001_REG_ALS_UV_CONF command!\n");
	...
	APS_LOG("PA12201001 ps PA12201001_REG_PS_CONF1_2 command!\n");

	databuf[0] = PA12201001_REG_PS_CONF3_MS;
	...
	APS_LOG("PA12201001 ps PA12201001_REG_PS_CONF3_MS command!\n");
	databuf[0] = PA12201001_REG_PS_CANC;
	databuf[1] = 0x00;
	databuf[2] = 0x00;
	res = PA12201001_i2c_master_operate(client, databuf, 0x3, I2C_FLAG_WRITE);
	if (res <= 0) {
	...
	APS_LOG("PA12201001 ps PA12201001_REG_PS_CANC command!\n");

	if (obj->hw.polling_mode_als == 0) {
		databuf[0] = PA12201001_REG_ALS_THDH;
		...
		databuf[0] = PA12201001_REG_ALS_THDL;
...
static int als_enable_nodata(int en)
{
	...
	APS_ERR("PA12201001_obj als enable value = %d\n", en);

	mutex_lock(&PA12201001_mutex);
	if (en)
		set_bit(CMC_BIT_ALS, &PA12201001_obj->enable);
	else
		clear_bit(CMC_BIT_ALS, &PA12201001_obj->enable);
	mutex_unlock(&PA12201001_mutex);
	if (!PA12201001_obj) {
		APS_ERR("PA12201001_obj is null!!\n");
		return -1;
...
static int ps_enable_nodata(int en)
{
	...
	APS_LOG("PA12201001_obj als enable value = %d\n", en);
	mutex_lock(&PA12201001_mutex);
	if (en)
		set_bit(CMC_BIT_PS, &PA12201001_obj->enable);
	else
		clear_bit(CMC_BIT_PS, &PA12201001_obj->enable);
	mutex_unlock(&PA12201001_mutex);
	if (!PA12201001_obj) {
		APS_ERR("PA12201001_obj is null!!\n");
		return -1;
...
static int ps_get_data(int *value, int *status)
{
	...
	err = PA12201001_read_ps(PA12201001_obj->client, &PA12201001_obj->ps);
	if (err) {
		err = -1;
	} else {
...
static int PA12201001_i2c_remove(struct i2c_client *client)
{
	...
	/*------------------------PA12201001 attribute file for debug--------------------------------------*/
	err = PA12201001_delete_attr(&(PA12201001_init_info.platform_diver_addr->driver));
	if (err)
		APS_ERR("PA12201001_delete_attr fail: %d\n", err);
	/*----------------------------------------------------------------------------------------*/
	/* misc_deregister(&PA12201001_device); */
	alsps_factory_device_deregister(&pa12201001_factory_device);
...
static int PA12201001_i2c_detect(struct i2c_client *client, struct i2c_board_info *info)
{
	strlcpy(info->type, PA12201001_DEV_NAME, sizeof(info->type));
	return 0;
