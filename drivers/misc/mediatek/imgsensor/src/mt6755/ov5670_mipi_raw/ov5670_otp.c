    ...
    if(g_cur_cam_sensor==1 || g_cur_cam_sensor ==2)
    {
        if(!checkVersion)
        {
			...
			{			
				...
				agold_camera_info[g_cur_cam_sensor-1].date[1] = OV5670_read_i2c(0x700F + 5*index);	//0x7014 0x7019 0x701E
				agold_camera_info[g_cur_cam_sensor-1].date[2] = OV5670_read_i2c(0x7010 + 5*index);	//0x7015 0x701A 0x701F
			}
