...
//extern void OV5670CheckLensVersion(kal_uint8 i2c_write_id);
extern void ov5670_otp_cali(void);
...
static void set_mirror_flip(kal_uint8 image_mirror)
{
	...
	/********************************************************
	   ...
	   *
	   ********************************************************/
	#if defined(AGOLD_IMGSENSOR_OV5670_H_MIRROR)
		image_mirror = IMAGE_H_MIRROR;
	#elif defined(AGOLD_IMGSENSOR_OV5670_V_MIRROR)
		image_mirror = IMAGE_V_MIRROR;
	#elif defined(AGOLD_IMGSENSOR_OV5670_HV_MIRROR)
	...
	while (imgsensor_info.i2c_addr_table[i] != 0xff) {
		...
		do {
			*sensor_id = ((read_cmos_sensor(0x300B) << 8) | read_cmos_sensor(0x300C));
			...
			{
				//OV5670CheckLensVersion(imgsensor.i2c_write_id);			
				LOG_INF("i2c write id: 0x%x, sensor id: 0x%x\n", imgsensor.i2c_write_id,*sensor_id);	  
				return ERROR_NONE;
			}	
			LOG_INF("Read sensor id fail,addr:0x%x, id:0x%x\n", imgsensor.i2c_write_id,*sensor_id);
			retry--;
		} while(retry > 0);
		i++;
		retry = 2;
