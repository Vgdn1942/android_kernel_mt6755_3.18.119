...
//static int read_otp_vendor(unsigned char writeid);
...
static kal_uint16 OV13850_R2A_write_i2c(u16 addr,u32 para)
{
...
void OV13850_otp_cali(unsigned char writeid)
{
...
static int read_otp(struct otp_struct *otp_ptr)
{
	...
	if((otp_flag & 0xc0) == 0x40) {
		...
		LOG_INF("OTP info in group 1");
	}
	else if((otp_flag & 0x30) == 0x10) {
		addr = 0x7229; // base address of info group 2
		LOG_INF("OTP info in group 2");
	}
	if(addr != 0) {
...
if(addr != 0) {
	...
	LOG_INF("(*otp_ptr).flag = 0x%x\n\
			(*otp_ptr).module_integrator_id = 0x%x\n\
			(*otp_ptr).lens_id = 0x%x\n\
			(*otp_ptr).production_year = %d\n\
			(*otp_ptr).production_month = %d\n\
			(*otp_ptr).production_day = %d\n\
			temp = 0x%x\n\
			(*otp_ptr).rg_ratio = 0x%x\n\
			(*otp_ptr).bg_ratio = 0x%x\n",
			(*otp_ptr).flag,
...
/*static int read_otp_vendor(unsigned char writeid)
{
	...
	if(addr != 0) {
		flag = 0xC0; // valid info and AWB in OTP
		module_integrator_id = OV13850_R2A_read_i2c(addr);
		lens_id = OV13850_R2A_read_i2c( addr + 1);
		production_year = OV13850_R2A_read_i2c( addr + 2);
		production_month = OV13850_R2A_read_i2c( addr + 3);
		production_day = OV13850_R2A_read_i2c(addr + 4);
		temp = OV13850_R2A_read_i2c(addr + 7);
		rg_ratio = (OV13850_R2A_read_i2c(addr + 5)<<2) + ((temp>>6) & 0x03);
		bg_ratio = (OV13850_R2A_read_i2c(addr + 6)<<2) + ((temp>>4) & 0x03);	
	}
	else {
		flag = 0x00; // not info in OTP
		module_integrator_id = 0;
		lens_id = 0;
		production_year = 0;
		production_month = 0;
		production_day = 0;
