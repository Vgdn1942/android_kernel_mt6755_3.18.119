...
extern bool read_imx298_SPC( BYTE* data );
extern bool read_imx298_DCC( kal_uint16 addr,BYTE* data, kal_uint32 size);
extern bool IMX298CheckLensVersion(void);
...
static SENSOR_WINSIZE_INFO_STRUCT imgsensor_winsize_info[10] =
{
 ...
 { 4656, 3496, 0000,  648, 4656, 2199, 1922,  722, 0000, 0000, 1922,  722, 0000, 0000, 1920,  720}, //hight speed video
 { 4656, 3496, 0000,  492, 4656, 2500, 2216,  834, 0000, 0000, 2216,  834, 0000, 0000, 1476,  834},// slim video
 { 4656, 3496, 0000, 0000, 4656, 3496, 2328, 1748, 0000, 0000, 2328, 1748, 0000, 0000, 2328, 1748}, // Custom1 (defaultuse preview)
 ...
 {0x03, 0x0a,   0x00,   0x10, 0x40, 0x00, 
  ...
  0x00, 0x36, 0x0B51, 0x0001, 0x03, 0x00, 0x0000, 0x0000},
...
/*
static kal_uint32 imx298_ATR(UINT16 DarkLimit, UINT16 OverExp)
{
...
static void preview_setting(void)
{
   ...
   /************PD 8*6 window*************/
	...
	write_cmos_sensor(0x3110,0x02); //733
	write_cmos_sensor(0x3111,0xDD);
	write_cmos_sensor(0x3112,0x02); //542
	write_cmos_sensor(0x3113,0x1E);
	write_cmos_sensor(0x3114,0x06);//1594
	write_cmos_sensor(0x3115,0x3A);
	write_cmos_sensor(0x3116,0x04); //1205
	write_cmos_sensor(0x3117,0xB5);
...
static void capture_setting_pdaf(kal_uint16 currefps)
{
   ...
   if(currefps==150)
   {
   ...
   else
	{
	   ...
	   //write_cmos_sensor(0x3107,0x01);
	   /************PD 8*6 window*************/
	   write_cmos_sensor(0x3110,0x05); //1466
	   write_cmos_sensor(0x3111,0xBA);
	   write_cmos_sensor(0x3112,0x04); //1084
	   write_cmos_sensor(0x3113,0x3C);
	   write_cmos_sensor(0x3114,0x0C);//3190
	   write_cmos_sensor(0x3115,0x76);
	   write_cmos_sensor(0x3116,0x09);//2412
...
static kal_uint32 get_imgsensor_id(UINT32 *sensor_id)
{
        ...
        do {
            *sensor_id = return_sensor_id();
            if (*sensor_id == imgsensor_info.sensor_id) {
				...
				IMX298CheckLensVersion();
                return ERROR_NONE;//xfl
            }
	...
	LOG_INF("get_imgsensor_id line:%d\n", __LINE__);
	//imx298_apply_SPC();
...
static kal_uint32 imx298_awb_gain(SET_SENSOR_AWB_GAIN *pSetSensorAWB)
{
...
UINT32 IMX298_MIPI_RAW_SensorInit(PSENSOR_FUNCTION_STRUCT *pfFunc)
{
