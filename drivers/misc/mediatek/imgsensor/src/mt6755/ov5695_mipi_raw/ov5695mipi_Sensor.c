...
static imgsensor_info_struct imgsensor_info = { 
	...
	.checksum_value = 0xeb6b65f9,//0x523c51f6,		//checksum value for Camera Auto Test
	...
	.pre = {
		...
		.mipi_data_lp2hs_settle_dc = 85,//85 unit , ns
...
static void ihdr_write_shutter_gain(kal_uint16 le, kal_uint16 se, kal_uint16 gain)
{
  ...
  write_cmos_sensor(0x3820, (read_cmos_sensor(0x3820) | 0x01));   //enable ihdr
...
static void set_mirror_flip(kal_uint8 image_mirror)
{
	...
	/********************************************************
	   ...
	   *
	   ********************************************************/
	#if defined(AGOLD_IMGSENSOR_OV5695_H_MIRROR)
	image_mirror ^= 0x01;
	#elif defined(AGOLD_IMGSENSOR_OV5695_V_MIRROR)
	image_mirror ^= 0x02;
	#elif defined(AGOLD_IMGSENSOR_OV5695_HV_MIRROR)
	image_mirror ^= 0x03;
	#endif
	switch (image_mirror) {
		case IMAGE_NORMAL:
	...
	while (imgsensor_info.i2c_addr_table[i] != 0xff) {
		...
		do {
			*sensor_id = ((read_cmos_sensor(0x300B) << 8) | read_cmos_sensor(0x300C));
			...
			LOG_INF("Read sensor id fail, id: 0x%x,sensorid : 0x%x\n", imgsensor.i2c_write_id,*sensor_id);
			retry--;
		} while(retry > 0);
		i++;
		retry = 2;
