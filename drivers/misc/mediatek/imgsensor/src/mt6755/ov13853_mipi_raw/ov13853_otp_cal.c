...
static unsigned char OV13853MIPI_WRITE_ID = 0X00;
...
int read_otp(struct otp_struct *otp_ptr)
{
	...
	OV13853_R2A_write_i2c(0x3d88, 0x72); // OTP start address
	OV13853_R2A_write_i2c(0x3d89, 0x20);
	OV13853_R2A_write_i2c(0x3d8A, 0x73); // OTP end address
	OV13853_R2A_write_i2c(0x3d8B, 0xBE);
	OV13853_R2A_write_i2c(0x3d81, 0x01); // load otp into buffer
	Delay(10);
	// OTP base information and WB calibration data
	otp_flag = OV13853_R2A_read_i2c(0x7220);
	...
	if(addr != 0) {
		...
		(*otp_ptr).module_integrator_id = OV13853_R2A_read_i2c(addr);
		(*otp_ptr).lens_id = OV13853_R2A_read_i2c( addr + 1);
		...
		temp = OV13853_R2A_read_i2c(addr + 7);
		(*otp_ptr).rg_ratio = (OV13853_R2A_read_i2c(addr + 5)<<2) + ((temp>>6) & 0x03);
		(*otp_ptr).bg_ratio = (OV13853_R2A_read_i2c(addr + 6)<<2) + ((temp>>4) & 0x03);	
	}
	else {
	...
	// OTP VCM Calibration
	otp_flag = OV13853_R2A_read_i2c(0x73ac);
	...
	if(addr != 0) {
		(*otp_ptr).flag |= 0x20;
		temp = OV13853_R2A_read_i2c(addr + 2);
		(* otp_ptr).VCM_start = (OV13853_R2A_read_i2c(addr)<<2) | ((temp>>6) & 0x03);
		(* otp_ptr).VCM_end = (OV13853_R2A_read_i2c(addr + 1) << 2) | ((temp>>4) & 0x03);
	...
	otp_flag = OV13853_R2A_read_i2c(0x7231);
...
if(addr != 0) {
		for(i=0;i<186;i++) {
			(* otp_ptr).lenc[i]= OV13853_R2A_read_i2c(addr + i);
		...
		if(Decode_13853R2A((*otp_ptr).lenc,  lenc_out))
		{
			for(i=0;i<360;i++) 
			{
		...
		(* otp_ptr).checksumLSC=OV13853_R2A_read_i2c(addr + 186);
		(* otp_ptr).checksumOTP=OV13853_R2A_read_i2c(addr + 187);
		(* otp_ptr).checksumTotal=OV13853_R2A_read_i2c(addr + 188);
	...
	{
		OV13853_R2A_write_i2c(i,0); // clear OTP buffer, recommended use continuous write to accelarate
	}
...
int apply_otp(struct otp_struct *otp_ptr)
{
	...
	if ((*otp_ptr).flag & 0x40) {
		...
		if (R_gain>0x400) {
			OV13853_R2A_write_i2c(0x5056, R_gain>>8);
			OV13853_R2A_write_i2c(0x5057, R_gain & 0x00ff);
		}

		if (G_gain>0x400) {
			OV13853_R2A_write_i2c(0x5058, G_gain>>8);
			OV13853_R2A_write_i2c(0x5059, G_gain & 0x00ff);
		}

		if (B_gain>0x400) {
			OV13853_R2A_write_i2c(0x505A, B_gain>>8);
			OV13853_R2A_write_i2c(0x505B, B_gain & 0x00ff);
		}
	}
	...
	if ((*otp_ptr).flag & 0x10) {
		...
		temp = OV13853_R2A_read_i2c(0x5000);
		temp = 0x01 | temp;
		OV13853_R2A_write_i2c(0x5000, temp);
		...
		/***For function Decode_13853R2A(unsigned char*pInBuf, unsigned char* pOutBuf),please refer to lc42.h***/
		if(Decode_13853R2A((*otp_ptr).lenc,  lenc_out))
		{
			for(i=0;i<360 ;i++) {
				LOG_INF(" apply OTP lenc_out[%d]:%x \n", i, lenc_out[i]);
				OV13853_R2A_write_i2c(0x5200 + i, lenc_out[i]);
			}
		}
	}
