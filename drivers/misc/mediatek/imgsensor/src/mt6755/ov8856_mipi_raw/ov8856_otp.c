...
#define OTP_MID               0x02
#define TRULY_TYPICAL_RG      0x140
#define TRULY_TYPICAL_BG      0x132
...
signed char otp_read_wb_group(void)
{
	...
	write_cmos_sensor(otp_addr, 0x00);
	write_cmos_sensor(otp_addr+1, 0x00);
	if (mid == OTP_MID)
	{
...
{
	unsigned short ctrl;
	ctrl = read_cmos_sensor(0x5002);
	ctrl = ctrl | 0x08;
	write_cmos_sensor(0x5002,ctrl);
}
void DPCFuncDisable(void)
{
	unsigned short ctrl;
...
void OV8856CheckLensVersion(int writeid)
{
    int index =0;
	unsigned short otp_addr=0;
