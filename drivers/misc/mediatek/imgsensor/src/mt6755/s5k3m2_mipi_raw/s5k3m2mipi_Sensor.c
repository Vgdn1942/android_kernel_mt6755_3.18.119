...
extern kal_bool S5K3M2CheckLensVersion(void);
extern kal_bool update_awb_gain(kal_uint8 i2c_write_id);
extern kal_bool read_s5k3m2_otp_eeprom(kal_uint16 addr, unsigned char * data, kal_uint32 size);
extern void read_otpmodule_id(kal_uint8 * module_id);
    ...
    while (imgsensor_info.i2c_addr_table[i] != 0xff) {
        ...
        do {
			...
			*sensor_id = read_cmos_sensor(0x6F12);
			read_otpmodule_id(&module_id);
			//*sensor_id = 0x30d2;
		#if defined(AGOLD_C220_S5K3M2_FOR_SUNNY)
		    	 read_otpmodule_id(&module_id);
	             *sensor_id += module_id;		
		#endif
			
			LOG_INF("zhouling imgsensor_info.sensor_id: 0x%x, sensor id: 0x%x\n", imgsensor_info.sensor_id,*sensor_id);  
			
            if (*sensor_id == imgsensor_info.sensor_id) {               
              LOG_INF("zhouling i2c write id: 0x%x, sensor id: 0x%x\n", imgsensor.i2c_write_id,*sensor_id);  
	   			if(checkVersion != 1)
				{
					read_otp = S5K3M2CheckLensVersion();

					if(!read_otp)
					{
						printk("[zhouling][s5k3m2] Read OTP failed!!---- \n");
					}else{
						checkVersion = 1;
					}
				}
...
//	kal_uint8 module_id=0;	
	...
	sensor_init();
	update_awb_gain(imgsensor.i2c_write_id);
...
//	kal_uint32 ret = 0;	
	...
	switch (feature_id) {
		...
		case SENSOR_FEATURE_GET_PDAF_DATA:	
			...
			read_s5k3m2_otp_eeprom((kal_uint16 )(*feature_data),(char*)(uintptr_t)(*(feature_data+1)),(kal_uint32)(*(feature_data+2)));
			break;			
