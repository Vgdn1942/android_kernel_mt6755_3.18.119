...
extern bool IMX230CheckLensVersion(void);									
...
static void preview_setting(void)
{
	...
	if(imgsensor.pdaf_mode == 1)
	{
        /*PDAF*/
           ...
           /*AREA MODE*/
           write_cmos_sensor(0x31B0,0x02);// Flexible output
           write_cmos_sensor(0x31B4,0x01);// AREA_EN_0 enable.
           /*PD_OUT_EN=1*/
           write_cmos_sensor(0x3123,0x01);
           /*Fixed area mode*/
           
           write_cmos_sensor(0x3158,0x03);
           write_cmos_sensor(0x3159,0x4A);// X start = 842
           write_cmos_sensor(0x315a,0x02);
           write_cmos_sensor(0x315b,0x6F);// Y start= 623
           write_cmos_sensor(0x315c,0x07);
           write_cmos_sensor(0x315d,0x26);//X end = 1830
           write_cmos_sensor(0x315e,0x05);
           write_cmos_sensor(0x315f,0x68);// Y end = 1384
...
static void preview_setting_HDR_ES2(void)
{
	...
	if(imgsensor.pdaf_mode == 1)
	{
		...
		write_cmos_sensor(0x3121,0x01);
		write_cmos_sensor(0x31B0,0x02);// flexible mode.
		write_cmos_sensor(0x31B4,0x01);// enable flexible area0
		write_cmos_sensor(0x3123,0x01);
		write_cmos_sensor(0x3158,0x03);// X start_H
		write_cmos_sensor(0x3159,0x4A);// X start_L =842
		write_cmos_sensor(0x315a,0x02);// Y start_H
		write_cmos_sensor(0x315b,0x6F);// Y start_L =623
		write_cmos_sensor(0x315c,0x07);//X end_H
		write_cmos_sensor(0x315d,0x26);//X end_L= 1830
		write_cmos_sensor(0x315e,0x05);// Y end_H
		write_cmos_sensor(0x315f,0x68);// Y end_L= 1384
...
static void capture_setting_pdaf(kal_uint16 currefps)
{
	...
	/*AREA MODE*/
		...
		/*PD_OUT_EN=1*/
		write_cmos_sensor(0x3123,0x01);
		write_cmos_sensor(0x3158,0x06);// X start_H
		write_cmos_sensor(0x3159,0x94);// X start_L =1684
		write_cmos_sensor(0x315a,0x04);// Y start_H
		write_cmos_sensor(0x315b,0xDE);// Y start_L =1246
		write_cmos_sensor(0x315c,0x0E);//X end_H
		write_cmos_sensor(0x315d,0x4B);//X end_L= 3659
		write_cmos_sensor(0x315e,0x0A);// Y end_H
		write_cmos_sensor(0x315f,0xD2);// Y end_L= 2770
	
		LOG_INF("capture_setting_pdaf set PDAF end\n");
