...
//DD
	...
	{
	case 0x00:
		printk("GC5005_OTP_DD is Empty !!\n");
		gc5005_otp_info.dd_flag = 0x00;
		break;
	case 0x01:	
		printk("GC5005_OTP_DD is Valid!!\n");
		...
		printk("GC5005_OTP_DD total_number=%d!!\n",total_number);
		...
		for(i=0; i<total_number; i++)
		{
			...
			{//Read OTP
				...
				x = ((dd1&0x0f)<<8) + dd0;
				y = (dd2<<4) + ((dd1&0xf0)>>4);
				printk("GC5005_OTP_DD : type = %d , x = %d , y = %d \n",type,x,y);
				printk("GC5005_OTP_DD : dd0 = %d , dd1 = %d , dd2 = %d \n",dd0,dd1,dd2);
...
*/				
			...
			{
				printk("GC5005_OTP_DD:check_id[%d] = %x,checkid error!!\n",i,check_dd_flag);
			}
		}
	...
	case 0x02:
	case 0x03:	
		printk("GC5005_OTP_DD is Invalid !!\n");
	...
	printk("GC5005_OTP : flag1 = 0x%x , flag_golden = 0x%x\n",flag1,flag_golden);
...
//INFO&WB
	for(index=0;index<2;index++)
	{
		...
		{
		case 0x00:
			printk("GC5005_OTP_INFO group%d is Empty !!\n", index + 1);
			break;
		case 0x01:
		...
		case 0x02:
		case 0x03:	
			printk("GC5005_OTP_INFO group%d is Invalid !!\n", index + 1);
			break;
		default :
			break;
		}
		...
		{
		case 0x00:
			printk("GC5005_OTP_WB group%d is Empty !!\n", index + 1);
			gc5005_otp_info.wb_flag = gc5005_otp_info.wb_flag|0x00;
			break;
		case 0x01:
			printk("GC5005_OTP_WB group%d is Valid !!\n", index + 1);						
			...
			if((0==gc5005_otp_info.rg_gain)||(0==gc5005_otp_info.bg_gain))
			{
				...
				printk("GC5005_OTP_WB group%d is Error ,wb rg or bg = 0!!\n", index + 1);							
			}
			else
			{
		...
		case 0x02:
		case 0x03:	
			printk("GC5005_OTP_WB group%d is Invalid !!\n", index + 1);			
			gc5005_otp_info.wb_flag = gc5005_otp_info.wb_flag|0x02;
			break;
		default :
			break;
		...
		{
		case 0x00:
			printk("GC5005_OTP_GOLDEN group%d is Empty !!\n", index + 1);
			gc5005_otp_info.golden_flag = gc5005_otp_info.golden_flag|0x00;					
			break;
		case 0x01:
			printk("GC5005_OTP_GOLDEN group%d is Valid !!\n", index + 1);						
			...
			if((0==gc5005_otp_info.golden_rg)||(0==gc5005_otp_info.golden_bg))
			{
				...
				printk("GC5005_OTP_GOLDEN group%d is Error,golden rg or bg = 0!!\n", index + 1);							
			}
			else
			{
		...
		case 0x02:
		case 0x03:	
			printk("GC5005_OTP_GOLDEN group%d is Invalid !!\n", index + 1);	
			gc5005_otp_info.golden_flag = gc5005_otp_info.golden_flag|0x02;			
			break;
		default :
			break;
	...
	printk("GC5005_OTP_INFO:module_id=0x%x\n",gc5005_otp_info.module_id);
	printk("GC5005_OTP_INFO:lens_id=0x%x\n",gc5005_otp_info.lens_id);
	printk("GC5005_OTP_INFO:vcm_id=0x%x\n",gc5005_otp_info.vcm_id);
	printk("GC5005_OTP_INFO:vcm_driver_id=0x%x\n",gc5005_otp_info.vcm_driver_id);
	printk("GC5005_OTP_INFO:data=%d-%d-%d\n",gc5005_otp_info.year,gc5005_otp_info.month,gc5005_otp_info.day);
	printk("GC5005_OTP_WB:r/g=0x%x\n",gc5005_otp_info.rg_gain);
	printk("GC5005_OTP_WB:b/g=0x%x\n",gc5005_otp_info.bg_gain);
	printk("GC5005_OTP_GOLDEN:golden_rg=0x%x\n",gc5005_otp_info.golden_rg);
	printk("GC5005_OTP_GOLDEN:golden_bg=0x%x\n",gc5005_otp_info.golden_bg);	
}
...
static void gc5005_gcore_update_awb(void)
{
	...
	if(0x01==(gc5005_otp_info.golden_flag&0x01))
	{
		...
		printk("GC5005_OTP_UPDATE_AWB:rg_typical = 0x%x , bg_typical = 0x%x\n",rg_typical,bg_typical);		
	}
